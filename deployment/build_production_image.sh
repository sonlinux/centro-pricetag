#!/bin/bash
echo "Use this to build a production image of the pricetag-uwsgi"
...
then
    ...
    cd ..
	git archive --format=tar.gz $VERSION django_project > deployment/docker/pricetag.tar.gz
    cd -
    ...
    echo "ADD pricetag.tar.gz /home/web" >> docker/Dockerfile-prod
    echo "CMD [\"uwsgi\", \"--ini\", \"/uwsgi.conf\"]" >> docker/Dockerfile-prod
    # Now build the image and tag it
    cd docker
	docker build -t alisonmukoma/pricetag-uwsgi:latest -f Dockerfile-prod .
    docker tag alisonmukoma/pricetag-uwsgi:latest alisonmukoma/pricetag-uwsgi:$VERSION
    docker push alisonmukoma/pricetag-uwsgi:latest
    docker push alisonmukoma/pricetag-uwsgi:$VERSION
    cd -
	# Clean up
